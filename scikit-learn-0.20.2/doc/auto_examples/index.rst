...
用于 scikit-learn 的五花八门的介绍性案例



.. raw:: html

    <div class="sphx-glr-thumbcontainer" tooltip="在合成数据上使用保序回归的示例。 保序回归发现一个函数的非递减近似，同时最小化训练数据的均方误差。 这种模型的好处是它不采取任何形式的目标函数，如线性。 为便于比较，还呈现了一种线性回归方法...">
...
.. raw:: html
    ...
    <div class="sphx-glr-thumbcontainer" tooltip="这个例子展示了二维数据集上不同异常检测算法的特点。数据集包含一个或两个模式(高密度区域) 来说明算法处理多模态数据(multimodal data)的能力。">
...
.. raw:: html
    ...
    <div class="sphx-glr-thumbcontainer" tooltip="RBF核的显式特征映射逼近的一个例子。">
...
关于 :mod:`sklearn.cluster.bicluster` 模块的案例。
...
各种分类器的预测概率的校准(the calibration of predicted probabilities of classifiers)的案例
...
关于如何把一些功能单一的estimators组合为复杂的变换器(tranformer)和管道流(pipelines)。
请看 :ref:`用户指南 <combining_estimators>` 中的对应部分。
...
关于模块 :mod:`sklearn.cross_decomposition` 的案列.
...
关于 :mod:`sklearn.datasets` 模块的案例.



.. raw:: html

    <div class="sphx-glr-thumbcontainer" tooltip="该数据集由 1797 个  8x8 的图像构成。 每张图像, 就像下面展示的那张一样, 对应着一个手写数字。 为了使用像这样的一个 8x8 图像, 我们要 首先将其变换为长度为 64 的特征...">
...
.. raw:: html
    ...
    <div class="sphx-glr-thumbcontainer" tooltip="绘制若干个随机产生的 2D 分类数据集 这个示例代码展示了函数 :func:`datasets.make_classification` :func:`datasets.make_blobs...">
...
.. raw:: html
    ...
    <div class="sphx-glr-thumbcontainer" tooltip="numpy.ndarray 的每一行对应一个样本，而四个特征构成的四列的顺序为: Sepal Length, Sepal Width, Petal Length , Petal Width ...">
...
.. raw:: html
    ...
    <div class="sphx-glr-thumbcontainer" tooltip="这个例子展示了 `datasets.make_multilabel_classification` 数据集生成器。 每个样本包含两个特征(总共有50个)，在两类中的每一个上都是不同的分布。">
...
关于模块 :mod:`sklearn.decomposition` 的案例.
...
关于模块 :mod:`sklearn.feature_selection` 的案例。
...
关于 :mod:`sklearn.gaussian_process` 模块的案例。
...
关于 :mod:`sklearn.linear_model` 模块的案例。
...
与 :mod:`sklearn.model_selection` 模块相关的案例。
...
关于 :mod:`sklearn.multioutput` 模块的案例。
...
关于 :mod:`sklearn.neural_network` 模块的案列。
...
.. raw:: html
    ...
    <div class="sphx-glr-thumbcontainer" tooltip="This example demonstrates the use of the Box-Cox and Yeo-Johnson transforms through :class:`pre...">
...
关于 :mod:`sklearn.semi_supervised` 模块的案列。
...
关于 :mod:`sklearn.feature_extraction.text` 模块的案例。
...
.. only :: html
 ...
 .. container:: sphx-glr-footer
  ...
  .. container:: sphx-glr-download
    ...
    :download:`Download all examples in Python source code: auto_examples_python.zip </C:/GitHub/sklearn-cn/scikit-learn-0.20.2/doc/auto_examples/auto_examples_python.zip>`



  .. container:: sphx-glr-download

    :download:`Download all examples in Jupyter notebooks: auto_examples_jupyter.zip </C:/GitHub/sklearn-cn/scikit-learn-0.20.2/doc/auto_examples/auto_examples_jupyter.zip>`
